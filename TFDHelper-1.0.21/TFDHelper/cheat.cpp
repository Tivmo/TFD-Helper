				...
				if (GWorld->Levels.IsValidIndex(i))
				{
					...
					for (int j = 0; j < Level->Actors.Num(); ++j)
					{
						...
						if (TrySpawnVaultLoot)
						{
							...
							{
								//std::cout << "Trying loot \n";
								GISystem = TFD::UM1LocalGameInstanceSubsystem::GetSystem(GWorld);
								if (GISystem && GISystem->MiniGamePlayer)
								{
									if (GISystem->MiniGamePlayer->CurrentMiniGame)
									{
										//std::cout << "There is a current minigame\n";
										TFD::AM1MiniGameActor* Mini = static_cast<TFD::AM1MiniGameActor*>(Actor);
										if (Mini->MiniGameTid.ID == GISystem->MiniGamePlayer->CurrentMiniGame->MiniGameTemplateId.ID)
										{
											//std::cout << "This actor has matching ID, dropping items!\n";
											Mini->ServerDropItems(LocalPlayerController);
										}
									}
								}
								continue;
							...
							if (Actor->IsA(TFD::AM1FieldInteractableActor_Hit::StaticClass()))
							{
								if (Actor->GetFullName().contains("VulgusBox"))
								{
									...
									if (TrySpawnGenericLoot)
										LocalPlayerController->ServerRequestFieldObjectDropItems(reinterpret_cast<TFD::AM1FieldInteractableActor*>(Actor));
							...
							|| Item->DropItemInfo.ItemBox.Type == TFD::EM1ItemType::Rune)
						{
							...
							if (!isFiltered)
							{
								...
								if (OnScreen && CFG::cfg_Loot_DrawItemBoxes)
								{
									...
									if (ODistance > 0)
									{
										float Width = 100.0 / ODistance;
										float Height = 100.0 / ODistance;
