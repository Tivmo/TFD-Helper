...
namespace Menu
{
	...
	void HandleKeybinds()
	{
		...
		if (!Menu::IsCurrentlyBindingKey())
		{
			...
			if (ImGui::IsKeyPressed(VK_UP))
				Cheat::CurrentPresetIndex = Cheat::CurrentPresetIndex - 1 < 0 ? 3 : Cheat::CurrentPresetIndex - 1;

			if (ImGui::IsKeyPressed(VK_DOWN))
				Cheat::CurrentPresetIndex = Cheat::CurrentPresetIndex + 1 > 3 ? 0 : Cheat::CurrentPresetIndex + 1;
		}
	}
	...
	void RenderMenu()
	{
		if (ShowMenu)
		{
			if (ImGui::Begin("TFDInternal", &ShowMenu, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize))
			{
				if (ImGui::BeginTabBar("Tabs"))
				{
					...
					if (ImGui::BeginTabItem("Aimbot"))
					{
						...
						if (ImGui::CollapsingHeader("Features", NULL))
						{
							if (ImGui::BeginTable("AimFeatures", 2, ImGuiTableFlags_SizingFixedSame))
							{
								...
								/*ImGui::Text("Enable No Spread: ");
								ImGui::TableNextColumn();
								if (ImGui::Checkbox("##EANS", &CFG::cfg_Aim_NoSpread))
								{
									Cheat::ToggleMemFeature(0);
								}
								ImGui::TableNextRow();
								ImGui::TableNextColumn();
								ImGui::Text("Enable No Recoil: ");
								ImGui::TableNextColumn();
								if (ImGui::Checkbox("##EANR", &CFG::cfg_Aim_NoRecoil))
								{
									Cheat::ToggleMemFeature(1);
								}*/
								ImGui::TableNextRow();
								ImGui::TableNextColumn();
					...
					if (ImGui::BeginTabItem("Extra"))
					{
						if (ImGui::BeginTable("Settings", 2, ImGuiTableFlags_SizingFixedSame))
						{
							...
							ImGui::TableNextColumn();
							ImGui::SliderFloat("##ESPSS", &CFG::cfg_ESP_ShapeSize, 1.0f, 10.0f);
							ImGui::TableNextRow();
							ImGui::TableNextColumn();
							ImGui::Text("Adjust Size: ");
							ImGui::TableNextColumn();
							if (ImGui::Button("-1", ImVec2(30, 30)))
								CFG::cfg_ESP_ShapeSize = (CFG::cfg_ESP_ShapeSize > 2.0f) ? CFG::cfg_ESP_ShapeSize - 1.0f : 1.0f;
							ImGui::SameLine();
							if (ImGui::Button("+1", ImVec2(30, 30)))
								CFG::cfg_ESP_ShapeSize = (CFG::cfg_ESP_ShapeSize < 10.0f) ? CFG::cfg_ESP_ShapeSize + 1.0f : 10.0f;
							ImGui::TableNextRow();
							ImGui::TableNextColumn();
							ImGui::Text("ESP Shape Distance Scale: ");
							ImGui::TableNextColumn();
							ImGui::SliderFloat("##ESPSDS", &CFG::cfg_ESP_ShapeDistanceScale, 1.0f, 100.0f);
							ImGui::EndTable();
						}
						ImGui::EndTabItem();
					}
